...
class HistoricalApi {
    ...
    async fetchEquityHistoricalData(params) {
        ...
        if (!from_date && !to_date && JSON.stringify(series) === JSON.stringify(["EQ"])) {
            const data = await this.httpClient.request(`${index_js_1.BASE_URL}/historical/cm/equity`, { symbol });
        ...
        for (const [cFrom, cTo] of chunks) {
            const data = await this.httpClient.request(`${index_js_1.BASE_URL}/historical/cm/equity`, {
                symbol,
    ...
    async fetchHistoricalVixData(params = {}) {
        ...
        const data = [];
        for (const [cFrom, cTo] of chunks) {
            const result = await this.httpClient.request(`${index_js_1.BASE_URL}/historical/vixhistory`, {
                from: (0, date_formatter_js_1.formatDateDMY)(cFrom),
                to: (0, date_formatter_js_1.formatDateDMY)(cTo),
            });
    ...
    async fetchHistoricalFnoData(params) {
        ...
        const data = [];
        for (const [cFrom, cTo] of chunks) {
            query.from = (0, date_formatter_js_1.formatDateDMY)(cFrom);
            query.to = (0, date_formatter_js_1.formatDateDMY)(cTo);
            const result = await this.httpClient.request(`${index_js_1.BASE_URL}/historical/foCPV`, query);
            data.push(...result.data);
        }
        return data;
    }
    ...
    async fetchHistoricalIndexData(params) {
        ...
        for (const [cFrom, cTo] of chunks) {
            const data = await this.httpClient.request(`${index_js_1.BASE_URL}/historical/indicesHistory`, {
